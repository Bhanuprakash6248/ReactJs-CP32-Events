...
class Events extends Component {
  state = {status: eventsList[0].registrationStatus, isClicked: false}

  toggleList = id => {
    const getStatus = eventsList.findIndex(each => each.id === id)
    this.setState({
      status: eventsList[getStatus].registrationStatus,
      isClicked: true,
    })
  }
